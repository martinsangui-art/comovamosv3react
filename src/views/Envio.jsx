...
function PreviewModal({ sede, campNombre, onClose, onSend }) {
  ...
  const handleSend = async () => {
    setEnviando(true)
    await onSend(sede, editando ? htmlCustom : null)
    setEnviando(false)
    onClose()
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button
            onClick={() => {
              if (!editando) setHtmlCustom(htmlBase)
              setEditando(e => !e)
            }}
            style={{
              padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600,
              border: '1px solid #e2e8f0', background: editando ? '#1B2A6B' : '#f8fafc',
              color: editando ? '#fff' : '#64748b', cursor: 'pointer',
            }}
          >
            {editando ? '👁 Ver preview' : '✏️ Editar HTML'}
          </button>
          <div style={{ flex: 1 }} />
