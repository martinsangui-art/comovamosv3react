...
const COLORS_ESTADO = { green: '#059669', amber: '#d97706', red: '#e11d48' }
...
function buildEmailHTML(d, campNombre) {
  const color = COLORS_ESTADO[getEstado(d)]
  const noav = d.var === 0
  const varTxt = d.var !== null
    ? (d.var > 0 ? ` (+${d.var} vs semana anterior)` : d.var < 0 ? ` (${d.var} vs semana anterior)` : ' (sin variación)')
    : ''
  const extra = noav
    ? `\n\nNoté que esta semana no hubo nuevos ingresos registrados. Estoy a disposición para conectarnos y ver juntos cómo puedo acompañarte — a veces una charla corta ayuda a destrabar cosas. Avisáme cuando tengas un momento y coordinamos.`
    : ''
  return `${d.saludo}:

Enviamos el resultado del cómo vamos al ${d.fecha}.

SEDE: ${d.sede}
OBJETIVO ${campNombre.toUpperCase()}: ${d.objetivo}
TOTAL ACUMULADO AL ${d.fecha}: ${d.total}${varTxt}
% CUMPLIMIENTO: ${d.pct}%
${extra}
...
function buildEmailHTMLRich(d, campNombre) {
  const color = COLORS_ESTADO[getEstado(d)]
  const noav = d.var === 0
  const varTxt = d.var !== null
    ? (d.var > 0 ? ` (+${d.var} vs semana anterior)` : d.var < 0 ? ` (${d.var} vs semana anterior)` : ' (sin variación)')
    ...
    <div style={{ position: 'relative', display: 'inline-block' }}
      ...
      {show && (
        <div style={{
          ...
          padding: '6px 12px', borderRadius: 6, zIndex: 99,
          width: 260, textAlign: 'center', lineHeight: 1.5, pointerEvents: 'none',
        }}>{text}</div>
      )}
    </div>
  )
...
// Modal de preview y edición de email
function PreviewModal({ sede, campNombre, onClose, onSend }) {
  const [texto, setTexto] = useState(() => buildEmailHTML(sede, campNombre))
  const [enviando, setEnviando] = useState(false)

  const handleSend = async () => {
    setEnviando(true)
    await onSend(sede, texto)
    setEnviando(false)
    onClose()
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ background: 'linear-gradient(135deg,#1B2A6B,#0f1d4a)', padding: '16px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div>
            <div style={{ color: '#fff', fontWeight: 700, fontSize: 15 }}>Vista previa y edición</div>
            <div style={{ color: 'rgba(255,255,255,.6)', fontSize: 12, marginTop: 2 }}>{sede.sede} · {sede.email}</div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
          {[
            ['Objetivo', sede.objetivo],
            ['Total', sede.total],
            ['Cumplimiento', sede.pct + '%'],
            ['Variación', sede.var !== null ? (sede.var > 0 ? '+' + sede.var : sede.var) : '—'],
          ].map(([l, v]) => (
            <div key={l} style={{ flex: 1, padding: '10px 16px', textAlign: 'center', borderRight: '1px solid #f1f5f9' }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: '#0f172a', fontVariantNumeric: 'tabular-nums' }}>{v}</div>
        ...
        <div style={{ padding: '16px 20px', flex: 1, overflow: 'auto' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
            Texto del email — podés editar antes de enviar
          </div>
          <textarea
            value={texto}
          ...
          <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 6 }}>
            Este texto se enviará vía Make desde mcrossi@ucasal.edu.ar como tabla HTML formateada.
          </div>
        </div>

        {/* Footer */}
        <div style={{ padding: '12px 20px', borderTop: '1px solid #e2e8f0', display: 'flex', gap: 8, justifyContent: 'flex-end', flexShrink: 0 }}>
          <button onClick={onClose} style={{ padding: '9px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: '#f8fafc', color: '#64748b', border: '1px solid #e2e8f0', cursor: 'pointer' }}>
            Cancelar
          </button>
...
export default function Envio({ data, copied, onCopied, campanas, campanaActiva, guardarSemana }) {
  ...
  const enviarUno = async (d, textoCustom) => {
    const htmlRich = buildEmailHTMLRich(d, camp?.nombre || '')
    await fetch(WEBHOOK, {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        emailHTML: htmlRich,
        emailTexto: textoCustom || buildEmailHTML(d, camp?.nombre || ''),
      }),
    })
    onCopied(d.cod_sede)
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {[
          ...
          <div key={s.l} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '18px 24px', borderTop: `3px solid ${s.c}` }}>
      ...
      {!cerrada && (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden' }}>
          ...
          {/* Lista de pendientes con selector + preview */}
          {pendientes.length > 0 && (
            <div style={{ padding: '16px 24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <TooltipHelp text="Seleccioná las sedes que querés enviar ahora. Sin selección se envían todas. Usá el ojo para previsualizar y editar cada email antes de enviar." />
                <div style={{ flex: 1 }} />
                  ...
                  return (
                    <div key={d.cod_sede} style={{
                      ...
                      <button
                        ...
                        title="Ver y editar email antes de enviar"
                        style={{
                          padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600,
                          border: '1px solid #e2e8f0', background: '#fff', color: '#64748b',
                          cursor: 'pointer', flexShrink: 0, transition: 'all 0.15s',
                        }}
                        onMouseEnter={e => { e.currentTarget.style.background = '#1B2A6B'; e.currentTarget.style.color = '#fff' }}
        ...
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', padding: '14px 24px', borderBottom: mostrarNueva ? '1px solid #e2e8f0' : 'none' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>Ingresá los totales del nuevo corte y guardá en Google Sheets</div>
            </div>
            <TooltipHelp text="Usá esta función al cierre de cada semana: ingresás cuántos alumnos registró cada sede y se guarda automáticamente en el historial de Google Sheets para que todos los análisis se actualicen." />
            <div style={{ width: 12 }} />
          ...
          {mostrarNueva && (
            <div className="animate-fadeIn" style={{ padding: '16px 24px' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={() => setMostrarNueva(false)} style={{ padding: '9px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: '#f8fafc', color: '#64748b', border: '1px solid #e2e8f0', cursor: 'pointer' }}>
                  Cancelar
                </button>
              </div>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 9500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 480, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.3)' }}>
            <div style={{ background: 'linear-gradient(135deg,#C8102E,#9b0d23)', padding: '18px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ color: '#fff', fontWeight: 700, fontSize: 16 }}>🚀 Confirmar envío masivo</div>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 16 }}>
                💡 Para revisar o editar un email antes de enviarlo, cerrá este modal y usá el botón "👁 Ver" en cada sede.
              </div>
